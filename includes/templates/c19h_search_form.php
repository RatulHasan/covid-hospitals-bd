<?php
/***
 * C19h Search form file
 *
 * @since 1.0.0
 *
 * @author Ratul Hasan <tanjilhasanratul@gmail.com>
 *
 * @package Covid\Hospitals
 */

?>
<form action="<?php echo esc_url_raw( get_permalink() ); ?>" method="get">
    <div class="col-md-8 offset-md-2 mt-4">
        <div class="input-group">
            <input autocomplete="off" value="<?php echo isset( $_GET['search'] )? sanitize_text_field( wp_unslash( $_GET['search'] ) ) : '';?>" name="search" type="search" class="form-control" placeholder="Search by district or hospital name" aria-label="Search by district or hospital name" aria-describedby="button-addon2">
            <button class="btn btn-outline-secondary" type="submit" id="button-addon2">
                <img src="data:image/svg+xml;base64," alt="Search">
            </button>
            <?php wp_nonce_field( 'c19h_search', 'c19h_search_field' ); ?>
        </div>
    </div>
</form>
